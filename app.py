...
if choice == 1:
    ...
    BID_coin_price = round(initial_coin_price + ((initial_coin_price * percent_increase) / 100), 2)
else:
    safe_BID_Value = input("Enter Safe High BID Coin Value             : ")
    safe_BID_Value = float(safe_BID_Value)
    percent_increase = round(((100 * (safe_BID_Value - initial_coin_price)) / initial_coin_price), 2)
    BID_coin_price = float(safe_BID_Value)



final_amt = round(initial_buying_amt + ((initial_buying_amt * percent_increase) / 100), 2)
sellingPrice_brokerage = round(final_amt - ((final_amt * 0.2) / 100), 2)
total_profit = round((sellingPrice_brokerage - initial_buying_amt), 2)
